...
CUSTOM_FILTERS = [
    {"indicator": "RSI", "period": 7, "timeframe": "5m", "condition": "<", "threshold": 60},
...
# Default rules (from mean_reversion.yaml)
DEFAULT_RULES = [
    {"pattern": "UUUUU", "side": "DOWN", "coins": ["ETH"],              "max_ask": 0.54},
    {"pattern": "UUUUU", "side": "DOWN", "coins": ["BTC","SOL","XRP"],  "max_ask": 0.51},
    {"pattern": "UUUU",  "side": "DOWN", "coins": ["ETH"],              "max_ask": 0.54},
...
def fetch_klines(symbol: str, interval: str, start_ms: int, end_ms: int) -> pd.DataFrame:
    ...
    while cur < end_ms:
        ...
        if not data:
            break
        for k in data:
            rows.append({"open_time": k[0], "open": float(k[1]), "high": float(k[2]),
...
def load_csvs() -> pd.DataFrame:
    ...
    for f in files:
        ...
        frames.append(df)
        print(f"    {Path(f).name}: {len(df):,} rows, {df['cycle_start'].nunique()} cycles")
...
def csv_build_cycles(raw: pd.DataFrame):
    ...
    coin_data = {c: {"outcomes": [], "dates": [], "start_ts": [], "entry_ask": []}
                 for c in COINS}
    ...
    for idx, (cycle_start, g) in enumerate(cycle_groups):
        ...
        entry = g[(g["seconds_elapsed"] >= ENTRY_WINDOW[0]) &
        ...
        for coin in COINS:
            ...
            if slug in cache:
                ...
                if api_calls % 100 == 0:
                    ...
                    if api_calls % 100 == 0:
                        print(f"    {api_calls} API calls, {idx+1}/{len(cycle_groups)} cycles...")
    ...
    print(f"  Outcomes: {n_resolved:,} resolved, {n_unknown:,} unknown "
          f"({n_resolved/total*100:.0f}% resolved)" if total else "")
    ...
    for coin, d in seqs.items():
        ...
        for i in range(1, n):
            ...
            for rule in rules:
                ...
                trades.append({
                    "coin": coin, "date": dates[i], "start_ts": int(ts_arr[i]),
                    "pattern": rule["pattern"], "side": side,
                    "max_ask": rule["max_ask"], "entry_ask": entry_ask,
                    "outcome": actual, "hit": hit, "pnl": pnl,
                })
...
def build_sequences(tdf):
    ...
    for coin in COINS:
        ...
        seqs[coin] = {"outcomes": c["outcome"].values, "dates": c["date"].values,
                       "start_ts": c["start_ts"].values, "n": len(c)}
    return seqs
    ...
    for coin, d in seqs.items():
        ...
        for i in range(1, n):
            for rule in rules:
                ...
                actual = out[i]
                hit = (side == "DOWN" and actual == "D") or (side == "UP" and actual == "U")
                pnl = ((1.0-ma)*size - fee*size) if hit else (-ma*size - fee*size)
                trades.append({"coin": coin, "date": dates[i], "start_ts": int(ts[i]),
                                "pattern": rule["pattern"], "side": side, "max_ask": ma,
                                "outcome": actual, "hit": hit, "pnl": pnl})
                break
    if not trades:
        return pd.DataFrame()
...
def _col_name(f):
    ind = f["indicator"]
    p = f.get("period")
    if ind == "RSI": return f"RSI_{p or 14}"
    if ind == "BB_UPPER": return f"BB_UPPER_{p or 20}"
    if ind == "BB_LOWER": return f"BB_LOWER_{p or 20}"
    if ind == "MACD_HIST": return "MACD_HIST"
    if ind == "STOCH_K": return f"STOCH_K_{p or 14}"
    if ind == "VOL_RATIO": return "VOL_RATIO"
    return ind
...
def stats(df):
    if df.empty:
        return {"n": 0, "wr": 0, "pnl": 0, "daily": 0, "dd": 0, "days": 0, "pos_days": 0}
    n = len(df)
    wins = df["hit"].sum()
    pnl = df["pnl"].sum()
    ...
    ndays = len(daily)
    return {"n": n, "wr": wins/n, "pnl": pnl, "daily": daily.mean() if ndays else 0,
            "dd": dd, "days": ndays, "pos_days": (daily > 0).sum()}


def print_stats(label, s):
    print(f"  {label}")
    print(f"    Trades: {s['n']:,}  |  WR: {s['wr']*100:.1f}%  |  PnL: ${s['pnl']:+,.2f}")
    print(f"    Daily: ${s['daily']:+.2f}/day  |  Max DD: ${s['dd']:.2f}  |  "
...
def plot_curves(results: List[Tuple[str, pd.DataFrame]], output_file: Path, title: str = ""):
    if not HAS_MPL:
    ...
    if use_cycles:
        ...
        for i, (label, tdf) in enumerate(results):
            ...
            ax.plot(cum_pnl.index, cum_pnl.values, color=color,
                    linewidth=2.0 if i == 0 else 1.5,
                    linestyle="-" if i < 4 else "--", label=short, alpha=0.9)

        ax.axhline(0, color="black", lw=0.5)
        ax.fill_between(range(max_cycles), 0,
                        [0]*max_cycles, alpha=0)  # dummy for layout
        ...
        for i, (label, tdf) in enumerate(results):
            if tdf.empty:
                continue
            daily = tdf.groupby("date").agg(pnl=("pnl","sum"), wr=("hit","mean"), n=("pnl","count")).reset_index()
            daily = daily.sort_values("date")
            daily["cum_pnl"] = daily["pnl"].cumsum()
            dates = pd.to_datetime(daily["date"])
            color = COLORS[i % len(COLORS)]
            s = stats(tdf)
            short = f"{label} ({s['n']:,}tr, {s['wr']*100:.0f}%WR, ${s['daily']:+.1f}/d)"
            ax_eq.plot(dates, daily["cum_pnl"], color=color, linewidth=2.0 if i==0 else 1.5,
                       linestyle="-" if i < 4 else "--", label=short, alpha=0.9)
        ...
        ax_eq.set_title(title or "Backtest Comparison — Telonex 74 days", fontsize=13, fontweight="bold")
...
def make_rules(uuu=0.51, uuuu=0.51, uuuuu=0.51, uduu=0.51, eth_bonus=0.03):
    ...
    """
    return [
        {"pattern": "UUUUU", "side": "DOWN", "coins": ["ETH"],              "max_ask": uuuuu + eth_bonus},
        {"pattern": "UUUUU", "side": "DOWN", "coins": ["BTC","SOL","XRP"],  "max_ask": uuuuu},
        {"pattern": "UUUU",  "side": "DOWN", "coins": ["ETH"],              "max_ask": uuuu + eth_bonus},
        {"pattern": "UUUU",  "side": "DOWN", "coins": ["BTC","SOL","XRP"],  "max_ask": uuuu},
        {"pattern": "UDUU",  "side": "DOWN", "coins": ["BTC"],              "max_ask": uduu},
        {"pattern": "UUU",   "side": "DOWN", "coins": ["BTC","ETH","SOL","XRP"], "max_ask": uuu},
    ]
...
STRATEGY_CURRENT = DEFAULT_RULES  # UUU=0.51, UUUU=0.51, ETH+0.03

STRATEGY_TIGHT = make_rules(      # Tighter UUU, keep rest
    uuu=0.49, uuuu=0.51, uuuuu=0.51, uduu=0.50, eth_bonus=0.03,
)

STRATEGY_AGGRESSIVE = make_rules(  # Wider for longer streaks
    uuu=0.49, uuuu=0.53, uuuuu=0.55, uduu=0.51, eth_bonus=0.04,
)

STRATEGY_CONSERVATIVE = make_rules(  # Only enter at good prices
    uuu=0.48, uuuu=0.49, uuuuu=0.50, uduu=0.49, eth_bonus=0.03,
)

STRATEGY_NO_UUU = [  # Drop UUU entirely (lowest edge), keep UUUU+
    {"pattern": "UUUUU", "side": "DOWN", "coins": ["ETH"],              "max_ask": 0.55},
    {"pattern": "UUUUU", "side": "DOWN", "coins": ["BTC","SOL","XRP"],  "max_ask": 0.52},
    {"pattern": "UUUU",  "side": "DOWN", "coins": ["ETH"],              "max_ask": 0.55},
    {"pattern": "UUUU",  "side": "DOWN", "coins": ["BTC","SOL","XRP"],  "max_ask": 0.52},
    {"pattern": "UDUU",  "side": "DOWN", "coins": ["BTC"],              "max_ask": 0.51},
]

RSI7_60 = [{"indicator":"RSI","period":7,"timeframe":"5m","condition":"<","threshold":60}]
RSI7_65 = [{"indicator":"RSI","period":7,"timeframe":"5m","condition":"<","threshold":65}]
RSI14_60 = [{"indicator":"RSI","period":14,"timeframe":"5m","condition":"<","threshold":60}]
...
PRESETS = {
    ...
    "rsi7": {
        ...
        "configs": [
            ...
            ("RSI(7) skip>55",                    DEFAULT_RULES, [{"indicator":"RSI","period":7,"timeframe":"5m","condition":"<","threshold":55}]),
            ("RSI(7) skip>60",                    DEFAULT_RULES, RSI7_60),
            ("RSI(7) skip>65",                    DEFAULT_RULES, RSI7_65),
            ("RSI(7) skip>70",                    DEFAULT_RULES, [{"indicator":"RSI","period":7,"timeframe":"5m","condition":"<","threshold":70}]),
        ],
    },
    "maxask": {
    ...
    "indicators": {
        ...
        "configs": [
            ("Baseline",                DEFAULT_RULES, []),
            ("RSI(7) skip>60",          DEFAULT_RULES, RSI7_60),
            ("BB above upper skip",     DEFAULT_RULES, [{"indicator":"BB_UPPER","period":20,"timeframe":"5m","condition":"<","threshold":0}]),
            ("MACD bullish skip",       DEFAULT_RULES, [{"indicator":"MACD_HIST","timeframe":"5m","condition":"<","threshold":0}]),
            ("Stoch >80 skip",          DEFAULT_RULES, [{"indicator":"STOCH_K","period":14,"timeframe":"5m","condition":"<","threshold":80}]),
            ("RSI(7)<60 + BB<upper",    DEFAULT_RULES, [
                {"indicator":"RSI","period":7,"timeframe":"5m","condition":"<","threshold":60},
                {"indicator":"BB_UPPER","period":20,"timeframe":"5m","condition":"<","threshold":0},
            ]),
        ],
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Backtest with equity curve comparison")
    ...
    parser.add_argument("--size", type=float, default=5.0, help="Trade size (default $5)")
    ...
    if args.custom:
        ...
        title = f"Custom Backtest ({mode_str}) | ${args.size}/trade"
    ...
    for label, rules, filters in configs:
        ...
        for r in rules:
            ...
            if key not in seen:
                print(f"    {r['pattern']:6} [{coins_str:10}] max_ask=${r['max_ask']:.2f}")
                seen.add(key)
        if filters:
            for f in filters:
                print(f"    filter: {f['indicator']}({f.get('period','')}) {f['condition']} {f['threshold']}")
    ...
    print("=" * 80)
    print(f"  {'Config':<30} {'Trades':>7} {'WR':>6} {'PnL':>10} {'$/day':>8} {'MaxDD':>8} {'ProfDays':>9}")
    print(f"  {'-'*30} {'-'*7} {'-'*6} {'-'*10} {'-'*8} {'-'*8} {'-'*9}")
    for label, tdf in results:
        s = stats(tdf)
        pd_str = f"{s['pos_days']}/{s['days']}" if s['days'] else "—"
        print(f"  {label:<30} {s['n']:>7,} {s['wr']*100:>5.1f}% ${s['pnl']:>+9.2f} "
              f"${s['daily']:>+7.2f} ${s['dd']:>7.2f} {pd_str:>9}")

    # Per-pattern breakdown for best config
    best_label, best_trades = max(results, key=lambda x: stats(x[1])["daily"] if not x[1].empty else -999)
    if not best_trades.empty:
        ...
        if has_entry_ask:
            ...
            print(f"  {'Pattern':<8} {'Trades':>7} {'WR':>6} {'PnL':>10} {'AvgAsk':>8} {'MinAsk':>8} {'MaxAsk':>8}")
            print(f"  {'-'*8} {'-'*7} {'-'*6} {'-'*10} {'-'*8} {'-'*8} {'-'*8}")
            for pat in sorted(best_trades["pattern"].unique()):
                sub = best_trades[best_trades["pattern"] == pat]
                n = len(sub); w = sub["hit"].sum()
                ea = sub["entry_ask"]
                print(f"  {pat:<8} {n:>7} {w/n*100:>5.1f}% ${sub['pnl'].sum():>+9.2f} "
